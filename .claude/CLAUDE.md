...
- **Deployment**: Hosted on Coolify against a server-side PostgreSQL database — **not** local-only SQLite as originally scoped. Still single-user (no auth, no multi-tenancy); the reversal is about *where* the data lives, not who uses it. **This deliberately reverses the original "SQLite everywhere / no server process" stack recommendation below** (quick task 260817-0qt, 2026-08-17) — the deployment model moved from local-only to hosted, and a server-side Postgres is the durable, hosted-friendly store that decision required. Kept here (not silently edited away) so the document stays honest about its own decision history.
- **Scheduling**: Requires a background scheduler for the daily pull
- **Tech stack**: PostgreSQL via psycopg 3 for storage (see Technology Stack below); everything else per the table below
...
| PostgreSQL via psycopg 3 | psycopg 3.3.x (`psycopg[binary]`) | Storage for raw items, daily snapshots, and computed rankings | **Reverses the original SQLite recommendation** (quick task 260817-0qt, 2026-08-17): the deployment model moved from local-only to hosted (Coolify), and a server-side Postgres is the durable, hosted-friendly store that requires. `pytest-postgresql` provisions an ephemeral cluster per test run from local PG binaries (no Docker) so the test suite still needs no long-lived database server. |
...
| ~~SQLite~~ **PostgreSQL (now the recommendation, not the alternative)** | ~~PostgreSQL~~ SQLite | **Superseded 2026-08-17 (quick task 260817-0qt):** the deployment model moved from local-only to hosted (Coolify), which is exactly the "remote-accessible, needs a server process anyway" condition this row originally named as the trigger to switch. Kept here as decision history, not current guidance — see the Technology Stack table and Constraints above for what's actually in use now. |
...
| ~~PostgreSQL~~ (superseded 2026-08-17, quick task 260817-0qt) | Was true when this tool was purely local; no longer applies now that deployment is hosted on Coolify and a server process is required regardless of the storage choice | PostgreSQL via psycopg 3 (see Technology Stack above) — SQLite is what to avoid now |
